    ...
    <Card className={cn('hover:shadow-md transition-shadow', className)}>
      ...
      {showActions && (
        <Card.Footer className="flex justify-end space-x-2">
          {onViewClick && (
            <Button size="sm" variant="outline" onClick={() => onViewClick(reservation.id)}>
              View
            </Button>
          )}

          {onEditClick && reservation.status !== 'cancelled' && (
            <Button size="sm" variant="outline" onClick={() => onEditClick(reservation.id)}>
              Edit
            </Button>
          )}

          {onCancelClick && reservation.status !== 'cancelled' && (
            <Button size="sm" variant="danger" onClick={handleCancelClick} isLoading={isLoading}>
              Cancel
            </Button>
          )}
        </Card.Footer>

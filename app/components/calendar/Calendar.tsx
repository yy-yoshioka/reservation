  ...
  const getDatesWithReservations = () => {
    ...
    timeSlots.forEach(slot => {
      if (slot.reservation) {
        ...
        const exists = dates.some(existingDate => 
  ...
  const getTitle = () => {
    ...
    if (currentView === 'day') {
      ...
      if (weekStart.getMonth() === weekEnd.getMonth()) {
        ...
        return `${weekStart.toLocaleDateString('en-US', { month: 'long' })} - ${weekEnd.toLocaleDateString('en-US', { month: 'long' })} ${weekStart.getFullYear()}`;
      }
    } else {
      options.year = 'numeric';
      options.month = 'long';
